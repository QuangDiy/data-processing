"""
BPEasyTokenizer - Wrapper around tiktoken for BPE encoding/decoding

Based on bpeasy library by Gautier Dag
Source: https://github.com/gautierdag/bpeasy
License: MIT
"""
...
# Import from the installed bpeasy package (not local source)
# Temporarily remove the local bpeasy directory from sys.path
_local_bpeasy_parent = str(Path(__file__).parent.parent)
_removed_from_path = False
if _local_bpeasy_parent in sys.path:
    sys.path.remove(_local_bpeasy_parent)
    _removed_from_path = True

try:
    import bpeasy as _bpeasy_pkg
finally:
    # Restore the path
    if _removed_from_path:
        sys.path.insert(0, _local_bpeasy_parent)
...
class BPEasyTokenizer:
    ...
    def train(
        ...
        bytes_vocab = _bpeasy_pkg.train_bpe(iterator, regex_pattern, max_token_length, vocab_size, batch_size)
        return cls(
            name=name,
            vocab=bytes_vocab,
